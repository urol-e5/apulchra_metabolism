...
## 2. Calculate correlation curves for all loggers  
...
light_output<-purrr::map(
  ...
  # here the .x is replace by a new variable 
  # in each iteration
  ~lm(`15643` ~ .x, data = df_light_cor)
  ) %>%

  # Then summarise each output with broom::tidy
  purrr::map(broom::tidy) %>%
...
```{r}
df_light_cor %>%
  gather(-`15643`, key = "var", value = "value") %>% 
  ggplot(aes(x = value, y = `15643`)) +
    facet_wrap(~ var, scales = "free") +
    geom_point() +
    ylab("Odyssey Logger Integrated PAR")+
    xlab("Pendant Logger Lux")+
    stat_smooth(method="lm")
```

The formula for calculating calibrations will be:  

(Logger value in lux * coef) + intercept = calibrated value in integrated PAR
